...
export class AuthService {
  ...
  initAuthListener() {
    this.angularFireAuth.authState.subscribe((fbUser: firebase.User) => {
      if (fbUser) {
        this.subscription = this.angularFireDB
          .doc(`${fbUser.uid}/${this.USER_COLLECTION}`)
          .valueChanges()
          .subscribe((user: User) => {
  ...
  createUser(name: string, email: string, password: string) {
    ...
    this.angularFireAuth
      ...
      .catch((err) => {
        ...
        swal.fire("Error al registrase", err.message, "error");
      });
  }
  ...
  private saveUserCollection(name: string, email: string, uid: string) {
    ...
    this.angularFireDB
      .doc(`${user.uid}/${this.USER_COLLECTION}`)
      .set(user)
      .then(() => {
